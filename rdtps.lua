...
function send(targetID, c, f, d, l, s, a, pk, h, ht) -- returns table { string, string, string, string, int }
  if debug then
    depth = depth + 1
    warning("send()")
  end
  ...
  local id
  local msg = { code = c, format = f, data = d, label = l, srcid = s, algorithm = a, pubkey = pk, hash = h, hashtype = ht }
...
function replyTo(targetID, c, f, d, l, s, a, pk, h, ht) -- returns nil
  if debug then
    depth = depth + 1
    warning("relyTo()")
  end
